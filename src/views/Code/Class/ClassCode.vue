  ...
  <div>
    <Card>
      <div style="min-height: 200px; text-align:left">
        ...
        <Steps :current="step.current" :status="step.status" direction="vertical" style="margin-top: 10px">
          <Step title="开始答题" content="正在将代码上传至判题系统">
            <Tabs :value="language" style="width: 230px;margin-left: 0px" @on-click="changeLanguage">
              <TabPane label="C" name="c" />
              <TabPane label="C++" name="c++" />
              <TabPane label="JAVA" name="java" disabled />
            </Tabs>
            <div style="width:70vw; height:605px;">
              <codemirror :options="cmOptions" v-model="answer" class="codetext"
            ...
            <div style="width:65vw; height:40px;display:inline-flex">
              <p style="width:55vw; height:34px; font-size: 13px;color: #ff0009">Tips: 答案最后不要输出回车;
                使用math库的同时要添加stdlib.h; 所有的输入必须在输出之前;</p>
              <Button style="width:10vw;height:34px;" icon="ios-cloud-upload-outline" type="primary" v-on:click="post">提
                交</Button>
            </div>
          </Step>
          ...
          <Step title="结果输出" content>
            <Table border :columns="columns1" :data="data1" class="my-table"
              style="margin-top:10px;margin-bottom: 20px;width:65vw;"></Table>
            <Card v-if="error.statue" style="width:65vw;margin-bottom: 30px;">
              ...
              <p v-for="message in error.message" :key="message.id"
                style="white-space: pre;font-size: 12px;font-family: Consolas">{{message}}</p>
            </Card>
          </Step>
        </Steps>
      </div>
